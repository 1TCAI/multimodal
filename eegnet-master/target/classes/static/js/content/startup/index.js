...
$(document).ready(function () {
    ...
    $("#btn_new").click(function () {
        window.location.href = "startup/update";
    });
    ...
    $("#btn_check_content").click(function () {
        ...
        if (!currentInformationId) {
            ...
            if (currentInformation.verify == 0) {
                $('#verifyDetail').html('');
                $.post('startup/selectById', { id: currentInformationId },
                    function (result) {
                        if (result.data) {
...
function bindData() {
    var table = $('#table_list').dataTable({
        ...
        columns: [{
            ...
            data: "os",
            title: '操作系统',
            "render" : function (os, type, full, meta) {
                var tnStr = '';
                switch (os) {
                    case 'I':
                        tnStr = '苹果';
            ...
            "render" : function (data, type, full, meta) {
                var str = "<button style='display: none' class='btn btn-info btn-xs edit' type='button' onclick=\"editRow('"+ data + "')\"><i class='fa fa-edit'></i> 编辑</button>  ";
                str = str + "<button style='display: none' class='btn btn-danger btn-xs delete' type='button' onclick=\"deleteRow('"+ data +"')\"><i class='fa fa-trash'></i> 删除</button>";
                return str;
            }
        }],
        columnDefs: [{
...
function verifyUrlFmatter(cellvalue) {
    if (cellvalue != null && cellvalue != "") {
        return '<a target="_blank" style="color:blue; text-decoration:underline;"'
            + 'href="' + cellvalue + '">' + cellvalue + '</a>';
    } else {
        return '暂无';
    }
    return cellvalue;
...
function imgUrlFmatter(cellvalue) {
    if (cellvalue != null && cellvalue != "") {
        return '<img style="width: 50px;height: 100px;" data-toggle="modal" href="#bannerImgModal" src=' + cellvalue + ' onclick=setImgUrl(\'' + cellvalue + '\') />';
    } else {
        return '暂无';
    }
    return cellvalue;
...
function editRow(id) {
    window.location.href = "startup/update?id=" + id;
}
